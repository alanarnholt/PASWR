...
#' that the distribution of \code{x} (in the one sample case) or of \code{x -
#' y} (in the paired two sample case) is symmetric about \code{mu} is
#' performed.
#' 
#' Otherwise, if both \code{x} and \code{y} are given and
#' \code{paired="FALSE"}, a Wilcoxon rank sum test is done.  In this case, the
#' null hypothesis is that the distributions of \code{x} and \code{y} differ by
#' a location shift of \code{mu} and the alternative is that they differ by
#' some other location shift (and the one-sided alternative \code{"greater"} is
#' that \code{x} is shifted to the right of \code{y}).
#' 
#' @param x is a numeric vector of data values.  Non-finite (i.e. infinite or
...
#' \code{"less"}.  The initial letter only may be given.
#' @param conf.level confidence level of the interval
#' @return A list with class \code{htest} containing the following components:
#' \item{statistic}{ the value of the test statistic with a name describing it}
#' \item{p.value}{the p-value for the test} \item{null.value}{the location
#' parameter \code{mu}} \item{alternative}{a character string describing the
...
#' @note The function is rather primitive and should only be used for problems
...
#' 
#' Myles Hollander & Douglas A. Wolfe (1999), \emph{Nonparametric Statistical
